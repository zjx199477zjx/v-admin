    ...
    <div>
        <div class="header-btn">
            ...
            <div class="page-addDifferent">
                <vxe-table
                    ...
                    <vxe-table-column title="2型糖尿病患者随访记录"  cell-type="string">
                        <template slot-scope="items">
                        <el-button  type="primary" size="small"  @click="addPeopleAsses(items)">添加</el-button>
                        </template>
                    </vxe-table-column>
                    <vxe-table-column title="2型糖尿病患者健康体检表"  cell-type="string">
                        <template slot-scope="items">
        ...
        <el-dialog :title="dialogTitle" :visible.sync="dialogFormVisible" :modal-append-to-body="false" width="60%">
            ...
            <template v-if="dialogState === 2">
                <fromViewHealthInfo ref="createFrom2" :record="scope" :stop="true"></fromViewHealthInfo>
            </template>
  ...
  export default {
    ...
    methods: {
      getDifferentList(value) {
        ...
        this.axios.post('SihealIMManagement/health_file/diabetes_manage_list', params).then(res => {
          res = res || {};
       ...
       saveFromInfo() {
           ...
           switch (this.dialogState) {
               case 1: 
                    ...
                    if (!params.followDate) return this.$message({type: 'error', message: '随访日期不能为空'});
                    if (!params.nextFollowDate) return this.$message({type: 'error', message: '下次随访日期不能为空'});
                    this.axios.post('SihealIMManagement/health_file/add_diabetes', params).then(res => {
               ...
               case 2: 
                    ...
                    params.type = 3;
                    params.tjid = '';
                    this.axios.post('SihealIMManagement/health_file/update_diabetes_exam', params).then(res => {
                        if (res.code !== 1) return this.$message({type: 'error', message: res.message || '操作异常'});
                        this.$message({type: 'success', message: '保存成功'});
                        this.dialogFormVisible = false;
                    })
